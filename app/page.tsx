...
export default function Home() {
  const [activeTab, setActiveTab] = useState<'garden' | 'admin'>('garden');
  
  const [adminConfig, setAdminConfig] = useState({
    mascotMessage: "Greetings! Let's cultivate focus under the twilight sky.",
    glowIntensity: 'medium' as 'low' | 'medium' | 'high',
    themePreset: 'twilight' as 'twilight' | 'midnight' | 'emerald',
    showGnome: true,
    showSparkles: true,
  });
  ...
  const themeBgMap = {
    twilight: 'from-[#1b3227] via-[#244534] to-[#15271f]',
    emerald: 'from-[#0f2e21] via-[#1a4a35] to-[#0c2419]',
    midnight: 'from-[#111827] via-[#1e293b] to-[#0f172a]',
  };

  return (
    <main className={`relative min-h-screen bg-gradient-to-b ${themeBgMap[adminConfig.themePreset]} flex flex-col items-center justify-between p-6 overflow-hidden select-none transition-colors duration-700`}>
      
      {/* Floating Ambient Fireflies */}
      {adminConfig.showSparkles && (
      ...
      <div className="relative z-10 w-full max-w-xl flex flex-col items-center justify-center my-6 gap-6">
        {activeTab === 'admin' ? (
          ...
          <>
            ...
            {/* Mascot Component */}
            <MascotMoonlight message={adminConfig.mascotMessage} />
          </>
        )}
      </div>
      ...
      <nav className="z-10 w-full max-w-sm bg-emerald-950/70 backdrop-blur-md border border-emerald-700/50 rounded-full py-3 px-6 flex justify-around text-xs font-semibold text-emerald-200 shadow-xl">
        <button
          ...
          className={`transition-colors ${activeTab === 'garden' ? 'text-glow-cyan font-bold' : 'hover:text-glow-cyan'}`}
        >
          GARDEN
        </button>
        <button
          onClick={() => setActiveTab('admin')}
          className={`transition-colors ${activeTab === 'admin' ? 'text-glow-cyan font-bold' : 'hover:text-glow-cyan'}`}
        >
          ⚙️ ADMIN
        </button>
      </nav>
